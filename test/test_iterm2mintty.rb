...
class Iterm2minttyTest < MiniTest::Test
  def setup
    @path = Pathname.new("test/Hybrid.itermcolors")
  end
...
ForegroundColour= 183, 188, 185
  ...
  def test_config_lines
    ...
    iterm2_theme = Iterm2mintty::Iterm2Theme.from_file(@path)
    theme_components = iterm2_theme.components
    mintty_theme = Iterm2mintty::MinttyTheme.new(theme_components)

    assert_equal config_lines.sort, mintty_theme.output_lines.sort
  end
end
